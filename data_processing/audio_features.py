...
def extract_sentences(file, script_dir="../data/GENEA/source/script/"):
    ...
    for sentence in script:
        sentences.append({
            "start_time:": sentence["alternatives"][0]["words"][0]["startTime"],
            "end_time": sentence["alternatives"][0]["words"][-1]["endTime"],
            "sentence": sentence["alternatives"][0]["transcript"]
        })
...
def extract_melspec(audio_dir, files, destpath, fps, sentence_path):
    for f in range(len(files)):
        ...
        for i in range(len(sentences)):
            ...
            features = librosa.feature.melspectrogram(y=X[start:end], sr=fs, n_fft=2048, hop_length=hop_len, n_mels=27,
                                                      fmin=0.0, fmax=8000)
